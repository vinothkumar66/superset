...
class PivotData {
  constructor(inputProps = {}, subtotals = {}) {
    ...
    this.getFormattedAggregator = this.getFormattedAggregator.bind(this);
    console.log(
      this.getFormattedAggregator,
      'console.log(this.getFormattedAggregator); ',
    );
    ...
    console.log(
      ...
      this.props,
      'this.props.aggregatorName,this props',
    );
    console.log(
      Array.isArray(this.props.aggregatorName),
      'aggregator name is array??',
    );
    ...
    //   }
    console.log('****************Entry aggregator func***********************');
    if (Array.isArray(this.props.aggregatorName)) {
      this.aggregator = this.props.aggregatorName.map(aggregatorName => {
      ...
      if (typeof aggregatorFunction === 'function') {
        this.aggregator = [aggregatorFunction(this.props.vals)];
      } else {
        console.error(
          `Aggregator function for "${this.props.aggregatorName}" not found.`,
        );
        this.aggregator = [null];
      }
    }

    if (this.props.customFormatters) {
      this.formattedAggregators = Object.entries(
        this.props.customFormatters,
      ).reduce((acc, [key, columnFormatter]) => {
        acc[key] = {};
        Object.entries(columnFormatter).forEach(([column, formatter]) => {
          acc[key][column] = this.props.aggregatorName.map(aggregatorName =>
            this.props
    ...
    // // Loop through each aggregator function
    // this.aggregator.forEach(aggregator => {
    //   console.log("check aggregator",aggregator)
    //   if (aggregator) {
    //     // Apply the aggregator to the data (replace `this` with the appropriate data input)
    //     const result = aggregator(this, [], []); // Adjust the parameters as needed
    //     this.allTotal.push(result); // Store the result in the allTotal array
    ...
    this.aggregator.forEach(aggregator => {
      if (typeof aggregator === 'function') {
        console.log('Applying aggregator:', aggregator);
        const result = aggregator(this, [], []); // Adjust parameters as needed
        this.allTotal.push(result);
      } else {
        console.error('Invalid aggregator function:', aggregator);
      }
    });
  ...
  processRecord(record) {
    const colKey = this.props.cols.map(col =>
      col in record ? record[col] : 'null',
    );
    const rowKey = this.props.rows.map(row =>
    ...
    this.processRowKeys(record, rowKey, rowStart);
    this.processColKeys(record, colKey, colStart);
    this.processTree(record, rowKey, colKey, rowStart, colStart);
  }

  processRowKeys(record, rowKey, rowStart) {
    for (let ri = rowStart; ri <= rowKey.length; ri += 1) {
      ...
      if (!this.rowTotals[flatRowKey]) {
        ...
        const aggregator = this.getFormattedAggregator(record, this.rowKeys[0]);
        if (typeof aggregator === 'function') {
          this.rowTotals[flatRowKey] = aggregator(this, fRowKey, []);
        } else {
          console.error(
            `Aggregator function for row key "${flatRowKey}" not found.`,
          );
      ...
      if (typeof this.rowTotals[flatRowKey].push === 'function') {
  ...
  processColKeys(record, colKey, colStart) {
  ...
  processTree(record, rowKey, colKey, rowStart, colStart) {
    for (let ri = rowStart; ri <= rowKey.length; ri += 1) {
      const fRowKey = rowKey.slice(0, ri);
      const flatRowKey = flatKey(fRowKey);
