...
class PivotData {
  constructor(inputProps = {}, subtotals = {}) {
    ...
    // Handle multiple aggregators
    // if (Array.isArray(aggregatorName)) {
    //   this.aggregators = aggregatorName.map(name =>
    //     aggregatorsFactory(defaultFormatter)[name](vals),
    //   );
    // } else {
    //   this.aggregators = [
    ...
    // }
    // Handle multiple aggregators
    this.aggregators = Array.isArray(aggregatorName)
      ? aggregatorName.map(name =>
          aggregatorsFactory(defaultFormatter)[name](vals),
        )
      : [aggregatorsFactory(defaultFormatter)[aggregatorName](vals)];
    ...
    this.formattedAggregators =
      customFormatters &&
      Object.entries(customFormatters).reduce((acc, [key, columnFormatter]) => {
        acc[key] = {};
        Object.entries(columnFormatter).forEach(([column, formatter]) => {
          acc[key][column] =
    ...
    this.rowTotals = {};
    this.colTotals = {};
    this.allTotal = this.aggregators[0]
      ? this.aggregators[0](this, [], [])
      : null;
  ...
  getFormattedAggregator(record, totalsKeys) {
    ...
    return (
      this.formattedAggregators[groupName][groupValue] || this.aggregators[0]
    );
  }
  ...
  processRecord(record) {
    ...
    for (let ri = rowStart; ri <= rowKey.length; ri += 1) {
      ...
      if (!this.rowTotals[flatRowKey]) {
        this.rowKeys.push(fRowKey);
        this.rowTotals[flatRowKey] = this.aggregators.map(agg =>
          agg(this, fRowKey, []),
        );
      }

      this.rowTotals[flatRowKey].forEach(agg => {
        // Use Object.assign to avoid direct parameter mutation
        Object.assign(agg, {
          isSubtotal: isRowSubtotal,
        });
        agg.push(record);
      });
    }
    ...
    for (let ci = colStart; ci <= colKey.length; ci += 1) {
      ...
      if (!this.colTotals[flatColKey]) {
        this.colKeys.push(fColKey);
        this.colTotals[flatColKey] = this.aggregators.map(agg =>
          agg(this, [], fColKey),
        );
      }

      this.colTotals[flatColKey].forEach(agg => {
        // Use Object.assign to avoid direct parameter mutation
        Object.assign(agg, {
          isSubtotal: isColSubtotal,
        });
        agg.push(record);
      });
    }
    ...
    for (let ri = rowStart; ri <= rowKey.length; ri += 1) {
      ...
      for (let ci = colStart; ci <= colKey.length; ci += 1) {
        ...
        if (!this.tree[flatRowKey][flatColKey]) {
          this.tree[flatRowKey][flatColKey] = this.aggregators.map(agg =>
            agg(this),
          );
        }

        this.tree[flatRowKey][flatColKey].forEach(agg => {
          // Use Object.assign to avoid direct parameter mutation
          Object.assign(agg, {
            isRowSubtotal,
            isColSubtotal,
            isSubtotal: isRowSubtotal || isColSubtotal,
          });
          agg.push(record);
  ...
  getAggregator(rowKey, colKey) {
    ...
    console.log(typeof agg, 'typeof(agg)');
    // console.log('All Total:', this.allTotal);
    // console.log('Column Totals:', this.colTotals);
    // console.log('Row Totals:', this.rowTotals);
    // console.log('Tree:', this.tree);
    console.log(this.aggregators[0], 'this.aggregators[0]');
    // Fallback to avoid the error if agg is undefined or doesn't have a value method
