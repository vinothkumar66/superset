...
const renderOperatorFields = ({ getFieldValue }: GetFieldValue) =>
  ...
  export const FormattingPopoverContent = ({
    ...
    const [localColorScheme, setLocalColorScheme] = useState(config?.colorScheme || '#fff');
    
    // Pickers' display control
    const [displayColorSchemePicker, setDisplayColorSchemePicker] = useState(false);
        ...
        <Row gutter={12}>
          <Col span={12}>
            <FormItem name="colorScheme" label={t('Background Color')}>
              <div>
                <div
                  onClick={() => setDisplayColorSchemePicker(!displayColorSchemePicker)}
                  style={{
                    width: '100%',
                    height: '35px',
