...
import { REPORTVIEWER_ROOT_ID } from './constants';
import { CHART_TYPE, REPORTVIEWER_ROOT_TYPE, TAB_TYPE } from './componentTypes';

const FILTER_SCOPE_CONTAINER_TYPES = [TAB_TYPE, REPORTVIEWER_ROOT_TYPE];
...
function traverse({
  ...
  if (FILTER_SCOPE_CONTAINER_TYPES.includes(type)) {
    let label = null;
    if (type === REPORTVIEWER_ROOT_TYPE) {
      label = t('All charts');
    } else {
      label =
...
export default function getFilterScopeNodesTree({
  ...
  const root = traverse({
    currentNode: components[REPORTVIEWER_ROOT_ID],
    components,
    filterFields,
    selectedChartId,
  });
