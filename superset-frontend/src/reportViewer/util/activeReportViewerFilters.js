...
} from './getReportViewerFilterKey';
import { CHART_TYPE } from './componentTypes';
import { REPORTVIEWER_FILTER_SCOPE_GLOBAL } from '../reducers/reportViewerFilters';
...
/**
 * @deprecated Please use src/reportViewer/util/getChartIdsInFilterScope instead
 */
export function getChartIdsInFilterScope({ filterScope }) {
  ...
  const { scope: scopeComponentIds, immune: immuneChartIds } =
    filterScope || REPORTVIEWER_FILTER_SCOPE_GLOBAL;
...
// non-empty filter fields in reportViewerFilters,
...
export function buildActiveFilters({ reportViewerFilters = {}, components = {} }) {
  // clear cache
  if (!isEmpty(components)) {
    allComponents = components;
  }
  appliedFilterValuesByChart = {};
  activeFilters = Object.values(reportViewerFilters).reduce((result, filter) => {
    ...
    Object.keys(columns).forEach(column => {
      if (
        ...
        nonEmptyFilters[getReportViewerFilterKey({ chartId, column })] = {
