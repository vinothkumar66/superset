...
const ownerUser: UserWithPermissionsAndRoles = {
  ...
  username: 'owner',
  permissions: {},
  roles: { Alpha: [['can_write', 'ReportViewer']] },
};
...
const reportViewer: ReportViewer = {
  id: 1,
  reportViewer_title: 'Test Report',
  url: 'https://reportViewer.example.com/1',
  thumbnail_url: 'https://reportViewer.example.com/1/thumbnail.png',
  published: true,
  css: null,
...
describe('canUserEditReportViewer', () => {
  it('allows owners to edit', () => {
    expect(canUserEditReportViewer(reportViewer, ownerUser)).toEqual(true);
  });
  it('allows admin users to edit regardless of ownership', () => {
    expect(canUserEditReportViewer(reportViewer, adminUser)).toEqual(true);
  });
  it('rejects non-owners', () => {
    expect(canUserEditReportViewer(reportViewer, outsiderUser)).toEqual(false);
  });
  it('rejects nonexistent users', () => {
    expect(canUserEditReportViewer(reportViewer, null)).toEqual(false);
  });
  it('rejects missing roles', () => {
    ...
    expect(canUserEditReportViewer(reportViewer, {})).toEqual(false);
  });
  it('rejects "admins" if the admin role does not have edit rights for some reason', () => {
    expect(
      canUserEditReportViewer(reportViewer, {
        ...adminUser,
        roles: { Admin: [] },
      }),
    ).toEqual(false);
...
describe('canUserSaveAsReportViewer with RBAC feature flag disabled', () => {
  beforeAll(() => {
    isFeatureEnabledMock = jest
      .spyOn(uiCore, 'isFeatureEnabled')
      .mockImplementation(
        (featureFlag: uiCore.FeatureFlag) =>
          featureFlag !== uiCore.FeatureFlag.ReportViewerRbac,
      );
  });
  ...
  it('allows owners', () => {
    expect(canUserSaveAsReportViewer(reportViewer, ownerUser)).toEqual(true);
  });

  it('allows admin users', () => {
    expect(canUserSaveAsReportViewer(reportViewer, adminUser)).toEqual(true);
  });

  it('allows non-owners', () => {
    expect(canUserSaveAsReportViewer(reportViewer, outsiderUser)).toEqual(true);
  });
});

describe('canUserSaveAsReportViewer with RBAC feature flag enabled', () => {
  beforeAll(() => {
    isFeatureEnabledMock = jest
      .spyOn(uiCore, 'isFeatureEnabled')
      .mockImplementation(
  ...
  it('reject non-owners', () => {
    expect(canUserSaveAsReportViewer(reportViewer, outsiderUser)).toEqual(false);
  });
});
