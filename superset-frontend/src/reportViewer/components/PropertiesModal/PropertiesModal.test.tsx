...
import * as ColorSchemeControlWrapper from 'src/reportViewer/components/ColorSchemeControlWrapper';
...
fetchMock.get(
  'http://localhost/api/v1/reportViewer/related/roles?q=(filter:%27%27,page:0,page_size:100)',
  {
    body: {
      count: 6,
      result: [
...
fetchMock.get(
  'http://localhost/api/v1/reportViewer/related/owners?q=(filter:%27%27,page:0,page_size:100)',
  {
    body: {
      count: 1,
      result: [
...
test('should render - FeatureFlag disabled', async () => {
  ...
  expect(
    await screen.findByTestId('reportViewer-edit-properties-form'),
  ).toBeInTheDocument();
...
test('should render - FeatureFlag enabled', async () => {
  ...
  expect(
    screen.getByRole('dialog', { name: 'ReportViewer properties' }),
  ).toBeInTheDocument();
...
test('submitting with onlyApply:false', async () => {
  ...
  put.mockResolvedValue({
    json: {
      result: {
        roles: 'roles',
        reportViewer_title: 'reportViewer_title',
        slug: 'slug',
  ...
  await waitFor(() => {
    ...
    expect(props.onSubmit).toBeCalledWith({
      ...
      owners: [],
      slug: '',
      title: 'COVID Vaccine ReportViewer',
    });
  });
});
...
test('should show all roles', async () => {
  ...
  const props = createProps();
  const propsWithReportViewerIndo = { ...props, reportViewerInfo };
  ...
  render(<PropertiesModal {...propsWithReportViewerIndo} />, {
    useRedux: true,
  });
...
test('should show active owners with reportViewer rbac', async () => {
  spyIsFeatureEnabled.mockReturnValue(true);
...
test('should show active owners without reportViewer rbac', async () => {
  spyIsFeatureEnabled.mockReturnValue(false);
