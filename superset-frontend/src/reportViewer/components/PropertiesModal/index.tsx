...
import ColorSchemeControlWrapper from 'src/reportViewer/components/ColorSchemeControlWrapper';
...
const PropertiesModal = ({
  ...
  reportViewerId,
  reportViewerInfo: currentReportViewerInfo,
  reportViewerTitle,
  onHide = () => {},
  onlyApply = false,
  onSubmit = () => {},
  ...
  const [reportViewerInfo, setReportViewerInfo] = useState<ReportViewerInfo>();
  ...
  const loadAccessOptions = useCallback(
    (accessType = 'owners', input = '', page: number, pageSize: number) => {
      ...
      return SupersetClient.get({
        endpoint: `/api/v1/reportViewer/related/${accessType}?q=${query}`,
      }).then(response => ({
  ...
  const handleReportViewerData = useCallback(
    reportViewerData => {
      const {
        id,
        reportViewer_title,
      ...
      const reportViewerInfo = {
        id,
        title: reportViewer_title,
        slug: slug || '',
      ...
      form.setFieldsValue(reportViewerInfo);
      setReportViewerInfo(reportViewerInfo);
      setOwners(owners);
      setRoles(roles);
  ...
  const fetchReportViewerDetails = useCallback(() => {
    setIsLoading(true);
    // We fetch the reportViewer details because not all code
    // that renders this component have all the values we need.
    // At some point when we have a more consistent frontend
    // datamodel, the reportViewer could probably just be passed as a prop.
    SupersetClient.get({
      endpoint: `/api/v1/reportViewer/${reportViewerId}`,
    }).then(response => {
      const reportViewer = response.json.result;
      const jsonMetadataObj = reportViewer.json_metadata?.length
        ? JSON.parse(reportViewer.json_metadata)
        : {};

      handleReportViewerData({
        ...reportViewer,
  ...
  }, [reportViewerId, handleReportViewerData]);
  ...
  const onFinish = () => {
    ...
    const morePutProps: { roles?: number[]; tags?: (number | undefined)[] } =
      {};
    if (isFeatureEnabled(FeatureFlag.ReportViewerRbac)) {
    ...
    if (onlyApply) {
      ...
      onHide();
      addSuccessToast(t('ReportViewer properties updated'));
    } else {
      SupersetClient.put({
        endpoint: `/api/v1/reportViewer/${reportViewerId}`,
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          reportViewer_title: title,
          slug: slug || null,
        ...
        onSubmit(onSubmitProps);
        onHide();
        addSuccessToast(t('The reportViewer has been saved'));
      }, handleErrorResponse);
    }
  };
      ...
      <Row gutter={16}>
        <Col xs={24} md={12}>
          ...
          <p className="help-block">
            {t(
              'Owners is a list of users who can alter the reportViewer. Searchable by name or username.',
            )}
          </p>
        </Col>
        <Col xs={24} md={12}>
      ...
      <>
        ...
        <Row gutter={16}>
          ...
          <Col xs={24} md={12}>
            ...
            <p className="help-block">
              {t(
                'Roles is a list which defines access to the reportViewer. Granting a role access to a reportViewer will bypass dataset level checks. If no roles are defined, regular access permissions apply.',
              )}
            </p>
          </Col>
        </Row>
  ...
  useEffect(() => {
    if (show) {
      if (!currentReportViewerInfo) {
        fetchReportViewerDetails();
      } else {
        handleReportViewerData(currentReportViewerInfo);
      }
    }

    JsonEditor.preload();
  }, [currentReportViewerInfo, fetchReportViewerDetails, handleReportViewerData, show]);

  useEffect(() => {
    // the title can be changed inline in the reportViewer, this catches it
    if (
      reportViewerTitle &&
      reportViewerInfo &&
      reportViewerInfo.title !== reportViewerTitle
    ) {
      form.setFieldsValue({
        ...reportViewerInfo,
  ...
  }, [reportViewerInfo, reportViewerTitle, form]);
  ...
  useEffect(() => {
    ...
    try {
      fetchTags(
        {
          objectType: OBJECT_TYPES.REPORTVIEWER,
          objectId: reportViewerId,
          includeTypes: false,
        },
  ...
  return (
    <Modal
      show={show}
      onHide={onHide}
      title={t('ReportViewer properties')}
      footer={
        <>
          <Button
            htmlType="button"
          ...
          <Button
            ...
            className="m-r-5"
            cta
            disabled={reportViewerInfo?.isManagedExternally}
            tooltip={
              reportViewerInfo?.isManagedExternally
                ? t(
                    "This reportViewer is managed externally, and can't be edited in Superset",
                  )
                : ''
            }
          >
    ...
    >
      <AntdForm
        form={form}
        onFinish={onFinish}
        data-test="reportViewer-edit-properties-form"
        layout="vertical"
        initialValues={reportViewerInfo}
      >
        <Row>
          <Col xs={24} md={24}>
        ...
        <Row gutter={16}>
          <Col xs={24} md={12}>
            <FormItem label={t('Name')} name="title">
              <Input
                data-test="reportViewer-title-input"
                type="text"
                disabled={isLoading}
              />
            </FormItem>
          ...
          <Col xs={24} md={12}>
            ...
            <p className="help-block">
              {t('A readable URL for your reportViewer')}
            </p>
          </Col>
        </Row>
        ...
        <Row gutter={16}>
          <Col xs={24} md={12}>
            ...
            <p className="help-block">
              {t('Person or group that has certified this reportViewer.')}
            </p>
          </Col>
          <Col xs={24} md={12}>
            <StyledFormItem
          ...
          <Col xs={24} md={24}>
            ...
            {isAdvancedOpen && (
              <>
                ...
                <p className="help-block">
                  {t(
                    'This JSON object is generated dynamically when clicking the save or overwrite button in the reportViewer view. It is exposed here for reference and for power users who may want to alter specific parameters.',
                  )}
                  {onlyApply && (
                    <>
                      {' '}
