...
const reportViewerResult = {
  json: {
    result: {
      reportViewer_title: 'New Title',
      slug: '/new',
...
fetchMock.get('glob:*/api/v1/reportViewer/related/owners?*', {
  result: [],
});

fetchMock.get('glob:*/api/v1/reportViewer/*', {
  result: {
    reportViewer_title: 'New Title',
    slug: '/new',
    json_metadata: '{"something":"foo"}',
...
// all these tests need to be moved to reportViewer/components/PropertiesModal/PropertiesModal.test.tsx
describe.skip('PropertiesModal', () => {
  ...
  describe('fetchReportViewerDetails', () => {
    it('should make an api call', () => {
      const spy = jest.spyOn(SupersetClient, 'get');
      const wrapper = setup();
      const modalInstance = wrapper.find('PropertiesModal').instance();
      modalInstance.fetchReportViewerDetails();
      expect(spy).toHaveBeenCalledWith({
    ...
    it('should update state', async () => {
      ...
      const fetchSpy = jest
        ...
        .mockResolvedValue(reportViewerResult);
      modalInstance.fetchReportViewerDetails();
      await fetchSpy();
      expect(modalInstance.state.values.colorScheme).toBeUndefined();
      expect(modalInstance.state.values.reportViewer_title).toEqual('New Title');
