...
const ReportViewerEmptyStateContainer = styled.div`
  position: absolute;
  top: 0;
  bottom: 0;
  left: 0;
...
class ReportViewerGrid extends PureComponent {
  constructor(props) {
    super(props);
    this.state = {
      isResizing: false,
  ...
  render() {
    ...
    const reportViewerEmptyState = editMode && (
      <EmptyStateBig
        title={t('Drag and drop components and charts to the reportViewer')}
        description={t(
        ...
        buttonAction={() => {
          window.open(
            `/chart/add?reportViewer_id=${reportViewerId}`,
            '_blank',
            'noopener noreferrer',
          );
        }}
    ...
    return width < 100 ? null : (
      <>
        {shouldDisplayEmptyState && (
          <ReportViewerEmptyStateContainer>
            {shouldDisplayTopLevelTabEmptyState
              ? topLevelTabEmptyState
              : reportViewerEmptyState}
          </ReportViewerEmptyStateContainer>
        )}
        <div className="reportViewer-grid" ref={this.setGridRef}>
          <GridContent
...
ReportViewerGrid.propTypes = propTypes;
ReportViewerGrid.defaultProps = defaultProps;

export default ReportViewerGrid;
