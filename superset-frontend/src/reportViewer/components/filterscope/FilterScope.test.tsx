...
const createProps = () => ({
  ...
  updateReportViewerFiltersScope: jest.fn(),
...
test('renders with empty filters', () => {
  render(<FilterScopeSelector {...createProps()} reportViewerFilters={{}} />, {
    useRedux: true,
  });
  expect(screen.getByText('Configure filter scopes')).toBeInTheDocument();
  expect(
    screen.getByText('There are no filters in this reportViewer.'),
  ).toBeInTheDocument();
...
test('triggers onSave', () => {
  const updateReportViewerFiltersScope = jest.fn();
  const setUnsavedChanges = jest.fn();
  const onCloseModal = jest.fn();
  render(
    <FilterScopeSelector
      {...createProps()}
      updateReportViewerFiltersScope={updateReportViewerFiltersScope}
  ...
  expect(updateReportViewerFiltersScope).toHaveBeenCalledTimes(0);
  expect(setUnsavedChanges).toHaveBeenCalledTimes(0);
  expect(onCloseModal).toHaveBeenCalledTimes(0);
  userEvent.click(screen.getByRole('button', { name: 'Save' }));
  expect(updateReportViewerFiltersScope).toHaveBeenCalledTimes(1);
