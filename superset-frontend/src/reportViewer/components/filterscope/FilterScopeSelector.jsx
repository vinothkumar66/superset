...
import buildFilterScopeTreeEntry from 'src/reportViewer/util/buildFilterScopeTreeEntry';
import getFilterScopeNodesTree from 'src/reportViewer/util/getFilterScopeNodesTree';
import getFilterFieldNodesTree from 'src/reportViewer/util/getFilterFieldNodesTree';
import getFilterScopeParentNodes from 'src/reportViewer/util/getFilterScopeParentNodes';
import getKeyForFilterScopeTree from 'src/reportViewer/util/getKeyForFilterScopeTree';
import getSelectedChartIdForFilterScopeTree from 'src/reportViewer/util/getSelectedChartIdForFilterScopeTree';
import getFilterScopeFromNodesTree from 'src/reportViewer/util/getFilterScopeFromNodesTree';
import getRevertedFilterScope from 'src/reportViewer/util/getRevertedFilterScope';
import { getChartIdsInFilterScope } from 'src/reportViewer/util/activeReportViewerFilters';
import {
  getChartIdAndColumnFromFilterKey,
  getReportViewerFilterKey,
} from 'src/reportViewer/util/getReportViewerFilterKey';
import { ALL_FILTERS_ROOT } from 'src/reportViewer/util/constants';
import { reportViewerFilterPropShape } from 'src/reportViewer/util/propShapes';
import FilterScopeTree from './FilterScopeTree';
import FilterFieldTree from './FilterFieldTree';

const propTypes = {
  reportViewerFilters: PropTypes.objectOf(reportViewerFilterPropShape).isRequired,
  layout: PropTypes.object.isRequired,

  updateReportViewerFiltersScope: PropTypes.func.isRequired,
...
export default class FilterScopeSelector extends PureComponent {
  constructor(props) {
    ...
    const { reportViewerFilters, layout } = props;

    if (Object.keys(reportViewerFilters).length > 0) {
      // display filter fields in tree structure
      const filterFieldNodes = getFilterFieldNodesTree({
        reportViewerFilters,
      });
      // filterFieldNodes root node is reportViewer_root component,
            ...
            (mapByChartId, columnName) => {
              ...
              const chartIdsInFilterScope = (
                getChartIdsInFilterScope({
                  filterScope: reportViewerFilters[filterId].scopes[columnName],
                }) || []
  ...
  onSave() {
    ...
    this.props.updateReportViewerFiltersScope(allFilterFieldScopes);
  ...
  renderEditingFiltersName() {
    const { reportViewerFilters } = this.props;
    ...
    const currentFilterLabels = []
      ...
      .map(key => {
        ...
        return reportViewerFilters[chartId].labels[column] || column;
      });
        ...
        <ScopeBody className="filter-scope-body">
          {!showSelector ? (
            <div className="warning-message">
              {t('There are no filters in this reportViewer.')}
            </div>
          ) : (
