...
// The types defined for Layout and sub elements is not compatible with the data we get back fro a real reportViewer layout
// This test file is using data from a real example reportViewer to test real world data sets.  ts-ignore is set for this entire file
...
describe('Ensure buildTree does not throw runtime errors when encountering an invalid node', () => {
  ...
  const layout: Layout = {
    ...
    'MARKDOWN-JOZKOjVc3a': {
      ...
      meta: {
        // @ts-expect-error
        code: '## 🎮Video Game Sales\n\nThis reportViewer visualizes sales & platform data on video games that sold more than 100k copies. The data was last updated in early 2017.\n\n[Original dataset](https://www.kaggle.com/gregorut/videogamesales)',
        height: 18,
        width: 12,
      },
      parents: [
