 ...
 * under the License.
 */
import { Charts, Layout, LayoutItem } from 'src/reportViewer/types';
import {
  CHART_TYPE,
...
export const isShowTypeInTree = ({ type }: LayoutItem) =>
  type === TAB_TYPE || type === CHART_TYPE || type === REPORTVIEWER_ROOT_TYPE;
...
export const buildTree = (
  ...
  if (
    ...
    node.type !== REPORTVIEWER_ROOT_TYPE &&
  ...
  node?.children?.forEach?.(child => {
    ...
    if (node) {
      ...
      logging.warn(
        `Unable to find item with id: ${child} in the reportViewer layout. This may indicate you have invalid references in your reportViewer and the references to id: ${child} should be removed.`,
      );
    }
  });
};
...
export const findFilterScope = (
  ...
  const checkedItemParents = checkedKeys
    ...
    .map(key => {
      const parents = [REPORTVIEWER_ROOT_ID, ...(layout[key]?.parents || [])];
  ...
  Object.entries(layout).forEach(([key, value]) => {
    ...
    if (
      ...
      [REPORTVIEWER_ROOT_ID, ...parents]?.find(parent => isExcluded(parent, key))
    ) {
...
export const isScopingAll = (scope: NativeFilterScope, chartId?: number) =>
  !scope ||
  (scope.rootPath[0] === REPORTVIEWER_ROOT_ID &&
