...
import { ReportViewerInfo, FilterBarOrientation } from 'src/reportViewer/types';
import * as mockedMessageActions from 'src/components/MessageToasts/actions';
import { FeatureFlag } from '@superset-ui/core';
import FilterBarSettings from '.';

const initialState: { reportViewerInfo: ReportViewerInfo } = {
  reportViewerInfo: {
    id: 1,
    userId: '1',
    metadata: {
...
const setup = (reportViewerInfoOverride: Partial<ReportViewerInfo> = {}) =>
  waitFor(() =>
    render(<FilterBarSettings />, {
      ...
      initialState: {
        ...initialState,
        reportViewerInfo: {
          ...initialState.reportViewerInfo,
          ...reportViewerInfoOverride,
        },
      },
    }),
  );
...
test('Dropdown trigger renders with reportViewer edit permissions', async () => {
  // @ts-ignore
  global.featureFlags = {
    [FeatureFlag.HorizontalFilterBar]: true,
  };
...
test('Dropdown trigger does not render without reportViewer edit permissions', async () => {
  // @ts-ignore
  global.featureFlags = {
    [FeatureFlag.HorizontalFilterBar]: true,
  };
...
test('Dropdown trigger renders with FF REPORTVIEWER_CROSS_FILTERS on', async () => {
  // @ts-ignore
  global.featureFlags = {
    [FeatureFlag.ReportViewerCrossFilters]: true,
  };
  await setup();
...
test('Dropdown trigger does not render with FF REPORTVIEWER_CROSS_FILTERS off', async () => {
  // @ts-ignore
  global.featureFlags = {
    [FeatureFlag.ReportViewerCrossFilters]: false,
  };
  await setup();
...
test('Can enable/disable cross-filtering', async () => {
  ...
  fetchMock.reset();
  fetchMock.put('glob:*/api/v1/reportViewer/1', {
    result: {},
  });
  await setup();
  userEvent.click(screen.getByLabelText('gear'));
  ...
  fetchMock.put('glob:*/api/v1/reportViewer/1', {
    result: {
      json_metadata: JSON.stringify({
        ...initialState.reportViewerInfo.metadata,
...
test('On failed request, restore previous selection', async () => {
  ...
  fetchMock.reset();
  fetchMock.put('glob:*/api/v1/reportViewer/1', 400);
  ...
  await waitFor(() => {
    expect(dangerToastSpy).toHaveBeenCalledWith(
      'Sorry, there was an error saving this reportViewer: Unknown Error',
    );
  });
