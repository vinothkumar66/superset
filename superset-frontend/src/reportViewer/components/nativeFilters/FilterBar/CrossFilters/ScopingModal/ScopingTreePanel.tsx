...
import ScopingTree from 'src/reportViewer/components/nativeFilters/FiltersConfigModal/FiltersConfigForm/FilterScope/ScopingTree';
import {
  ChartConfiguration,
  ReportViewerLayout,
...
const ChartSelect = ({
  ...
  const layout = useSelector<RootState, ReportViewerLayout>(
...
export const ScopingTreePanel = ({
  ...
  const isCrossFiltersEnabled = useSelector<RootState, boolean>(
    ({ reportViewerInfo }) => reportViewerInfo.crossFiltersEnabled,
  );
  return (
    <div
    ...
    >
        ...
        <Alert
          message={
            <span
              ...
              `}
            >
              {t('Cross-filtering is not enabled in this reportViewer')}
            </span>
          }
          type="info"
          closable={false}
      ...
      <InfoText>
        {isDefined(chartId)
          ? t(
              `Select the charts to which you want to apply cross-filters when interacting with this chart. You can select "All charts" to apply filters to all charts that use the same dataset or contain the same column name in the reportViewer.`,
            )
          : t(
              `Select the charts to which you want to apply cross-filters in this reportViewer. Deselecting a chart will exclude it from being filtered when applying cross-filters from any chart on the reportViewer. You can select "All charts" to apply cross-filters to all charts that use the same dataset or contain the same column name in the reportViewer.`,
            )}
      </InfoText>
      <ScopingTree
