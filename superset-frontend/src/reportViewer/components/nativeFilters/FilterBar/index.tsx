...
import { LOG_ACTIONS_CHANGE_REPORTVIEWER_FILTER } from 'src/logger/LogUtils';
import { FilterBarOrientation, RootState } from 'src/reportViewer/types';
...
const publishDataMask = debounce(
  async (
    ...
    if (
      ...
      dataMaskKey = await createFilterKey(reportViewerId, dataMask, tabId);
    }
    if (dataMaskKey) {
    ...
    // replace params only when current page is /superset/reportViewer
    // this prevents a race condition between updating filters and navigating to Explore
    if (window.location.pathname.includes('/superset/reportViewer')) {
...
const FilterBar: FC<FiltersBarProps> = ({
  ...
  const reportViewerId = useSelector<any, number>(
    ({ reportViewerInfo }) => reportViewerInfo?.id,
  );
  const previousReportViewerId = usePrevious(reportViewerId);
  ...
  useEffect(() => {
    if (previousFilters && reportViewerId === previousReportViewerId) {
      const updates = {};
  ...
  useEffect(() => {
    ...
    if (user?.userId) {
      publishDataMask(history, reportViewerId, updateKey, dataMaskApplied, tabId);
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [reportViewerId, dataMaskAppliedText, history, updateKey, tabId]);

  const handleApply = useCallback(() => {
    dispatch(logEvent(LOG_ACTIONS_CHANGE_REPORTVIEWER_FILTER, {}));
