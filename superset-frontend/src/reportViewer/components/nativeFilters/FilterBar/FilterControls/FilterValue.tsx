...
const useShouldFilterRefresh = () => {
  const isReportViewerRefreshing = useSelector<RootState, boolean>(
    state => state.reportViewerState.isRefreshing,
  );
  const isFilterRefreshing = useSelector<RootState, boolean>(
    state => state.reportViewerState.isFiltersRefreshing,
  );

  // trigger filter requests only after charts requests were triggered
  return !isReportViewerRefreshing && isFilterRefreshing;
};
...
const FilterValue: FC<FilterControlProps> = ({
  ...
  const reportViewerId = useSelector<RootState, number>(
    state => state.reportViewerInfo.id,
  );
