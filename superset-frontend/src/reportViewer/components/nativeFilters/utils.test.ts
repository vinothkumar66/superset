    ...
    beforeAll(() => {
      isFeatureEnabledMock = jest
        ...
        .mockImplementation((featureFlag: FeatureFlag) =>
          [FeatureFlag.ReportViewerCrossFilters].includes(featureFlag),
        );
    });
...
test('findTabsWithChartsInScope should handle a recursive layout structure', () => {
  ...
  } as any as ReportViewerLayout;

  expect(Array.from(findTabsWithChartsInScope(reportViewerLayout, []))).toEqual(
    [],
  );
});
