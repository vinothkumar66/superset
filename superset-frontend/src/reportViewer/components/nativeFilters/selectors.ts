...
export const getCrossFilterIndicator = (
  ...
  const reportViewerLayoutItem = Object.values(reportViewerLayout).find(
    layoutItem => layoutItem?.meta?.chartId === chartId,
  );
  const filterObject: Indicator = {
    column,
    name:
      reportViewerLayoutItem?.meta?.sliceNameOverride ||
      reportViewerLayoutItem?.meta?.sliceName ||
      '',
    path: [
      ...(reportViewerLayoutItem?.parents ?? []),
      reportViewerLayoutItem?.id || '',
    ],
    value: label,
  };
  return filterObject;
...
const cachedReportViewerFilterDataForChart = {};
...
export const selectIndicatorsForChart = (
  ...
  const cachedFilterData = cachedReportViewerFilterDataForChart[chartId];
  if (
  ...
  cachedReportViewerFilterDataForChart[chartId] = {
    appliedColumns,
    rejectedColumns,
    matchingFilters,
    matchingDatasources,
...
export const selectNativeIndicatorsForChart = (
  ...
  if (
    ...
    cachedFilterData?.reportViewerLayout === reportViewerLayout &&
