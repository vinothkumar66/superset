...
import { Layout, LayoutItem, RootState } from 'src/reportViewer/types';
...
import { useChartIds } from 'src/reportViewer/util/charts/useChartIds';
...
export const useFilterScope = (filter: Filter) => {
  ...
  return useMemo(() => {
    ...
    const topElementId = layout[REPORTVIEWER_ROOT_ID].children[0];
    ...
    if (
      ...
      (filter.scope.rootPath[0] === REPORTVIEWER_ROOT_ID ||
    ...
    if (filter.scope.rootPath[0] === REPORTVIEWER_ROOT_ID) {
      return {
        charts: chartIds
