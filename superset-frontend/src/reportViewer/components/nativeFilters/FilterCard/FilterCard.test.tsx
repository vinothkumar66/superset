...
test('filter card scope with excluded', () => {
  const filter = {
    ...baseFilter,
    scope: { rootPath: [REPORTVIEWER_ROOT_ID], excluded: [1, 4] },
  };
...
test('edit filter button for reportViewer viewer', () => {
  renderContent();
  expect(
...
test('edit filter button for reportViewer editor', () => {
  renderContent(baseFilter, {
    ...baseInitialState,
    reportViewerInfo: { reportedit_perm: true },
  });
