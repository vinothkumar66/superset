...
import { HeaderDropdownProps } from 'src/reportViewer/components/Header/types';
...
const editModeOnWithFilterScopesProps = {
  ...editModeOnProps,
  reportViewerInfo: {
    ...editModeOnProps.reportViewerInfo,
    metadata: {
      filter_scopes: {
...
function setup(props: HeaderDropdownProps) {
  return render(
    <div className="reportViewer-header">
...
test('should render the menu items', async () => {
  ...
  expect(screen.getByText('Refresh reportViewer')).toBeInTheDocument();
...
test('should render the "Refresh reportViewer" menu item as disabled when loading', async () => {
  const mockedProps = createProps();
  const loadingProps = {
    ...mockedProps,
    isLoading: true,
  ...
  setup(loadingProps);
  expect(screen.getByText('Refresh reportViewer')).toHaveClass(
    'ant-menu-item-disabled',
  );
});

test('should NOT render the "Refresh reportViewer" menu item as disabled', async () => {
  const mockedProps = createProps();
  setup(mockedProps);
  expect(screen.getByText('Refresh reportViewer')).not.toHaveClass(
    'ant-menu-item-disabled',
  );
});
