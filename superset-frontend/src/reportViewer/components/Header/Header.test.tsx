...
const createProps = () => ({
  ...
  reportViewerInfo: {
    id: 1,
    reportedit_perm: false,
    reportsave_perm: false,
    reportshare_perm: false,
    userId: '1',
    metadata: {},
    common: {
      conf: {
        REPORTVIEWER_AUTO_REFRESH_INTERVALS: [
          [0, "Don't refresh"],
          [10, '10 seconds'],
        ],
      },
  ...
  reportViewers: {},
  reportViewerTitle: 'ReportViewer Title',
  charts: {},
  layout: {},
  expandedSlices: {},
  css: '',
  ...
  isPublished: false,
  updateReportViewerTitle: jest.fn(),
  editMode: false,
  setEditMode: jest.fn(),
  ...
  reportViewerInfoChanged: jest.fn(),
  reportViewerTitleChanged: jest.fn(),
...
const editableProps = {
  ...
  editMode: true,
  reportViewerInfo: {
    ...props.reportViewerInfo,
    reportedit_perm: true,
    reportsave_perm: true,
  },
};
const undoProps = {
  ...editableProps,
...
test('should render the editable title', () => {
  setup(editableProps);
  expect(screen.getByDisplayValue('ReportViewer Title')).toBeInTheDocument();
});

test('should edit the title', () => {
  setup(editableProps);
  const editableTitle = screen.getByDisplayValue('ReportViewer Title');
...
test('should render metadata', () => {
  ...
  setup(mockedProps);
  expect(
    screen.getByText(getOwnerName(mockedProps.reportViewerInfo.created_by)),
  ).toBeInTheDocument();
  expect(
    screen.getByText(mockedProps.reportViewerInfo.changed_on_delta_humanized),
  ).toBeInTheDocument();
});
...
test('should toggle the edit mode', () => {
  ...
  setup(canEditProps);
  const editReportViewer = screen.getByText('Edit reportViewer');
  expect(screen.queryByText('Edit reportViewer')).toBeInTheDocument();
  userEvent.click(editReportViewer);
...
test('should refresh the charts', async () => {
  ...
  userEvent.click(screen.getByText('Refresh reportViewer'));
...
test('should render an extension component if one is supplied', () => {
  ...
  extensionsRegistry.set('reportViewer.nav.right', () => (
    <>reportViewer.nav.right extension component</>
  ));
  setupExtensions();
  ...
  setup(mockedProps);
  expect(
    screen.getByText('reportViewer.nav.right extension component'),
  ).toBeInTheDocument();
});
