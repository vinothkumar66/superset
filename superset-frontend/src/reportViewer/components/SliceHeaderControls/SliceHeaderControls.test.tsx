...
test('Should not show export full CSV if reportViewer is not table', async () => {
  (global as any).featureFlags = {
    [FeatureFlag.AllowFullCsvExport]: true,
  };
  renderWrapper();
...
test('Should not show export full Excel if reportViewer is not table', async () => {
  (global as any).featureFlags = {
    [FeatureFlag.AllowFullCsvExport]: true,
  };
  renderWrapper();
...
test('Should not show "Drill to detail" with neither of required perms', () => {
  ...
  props.slice.slice_id = 18;
  renderWrapper(props, {
    Admin: [['invalid_permission', 'ReportViewer']],
  });
  expect(screen.queryByText('Drill to detail')).not.toBeInTheDocument();
});
...
test('Should show "View query"', () => {
  ...
  props.slice.slice_id = 18;
  renderWrapper(props, {
    Admin: [['can_view_query', 'ReportViewer']],
  });
...
test('Should show "View as table"', () => {
  ...
  props.slice.slice_id = 18;
  renderWrapper(props, {
    Admin: [['can_view_chart_as_table', 'ReportViewer']],
  });
...
test('Should not show the "Edit chart" button', () => {
  ...
  renderWrapper(props, {
    Admin: [
      ['can_samples', 'Datasource'],
      ['can_view_query', 'ReportViewer'],
      ['can_view_chart_as_table', 'ReportViewer'],
    ],
  });
