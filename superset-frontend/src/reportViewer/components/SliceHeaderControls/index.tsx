...
import { getSliceHeaderTooltip } from 'src/reportViewer/util/getSliceHeaderTooltip';
import { Tooltip } from 'src/components/Tooltip';
import Icons from 'src/components/Icons';
import ModalTrigger from 'src/components/ModalTrigger';
import Button from 'src/components/Button';
import ViewQueryModal from 'src/explore/components/controls/ViewQueryModal';
import { ResultsPaneOnReportViewer } from 'src/explore/components/DataTablesPane';
import Modal from 'src/components/Modal';
import { DrillDetailMenuItems } from 'src/components/Chart/DrillDetail';
import { LOG_ACTIONS_CHART_DOWNLOAD_AS_IMAGE } from 'src/logger/LogUtils';
import { MenuKeys, RootState } from 'src/reportViewer/types';
...
export interface SliceHeaderControlsProps {
  ...
  forceRefresh: (sliceId: number, reportViewerId: number) => void;
...
const SliceHeaderControls = (props: SliceHeaderControlsPropsWithRouter) => {
  ...
  const canDrill = useSelector((state: RootState) =>
    findPermission('can_drill', 'ReportViewer', state.user?.roles),
  );
  const canDrillToDetail = (canExplore || canDrill) && canDatasourceSamples;
  const canViewQuery = useSelector((state: RootState) =>
    findPermission('can_view_query', 'ReportViewer', state.user?.roles),
  );
  const canViewTable = useSelector((state: RootState) =>
    findPermission('can_view_chart_as_table', 'ReportViewer', state.user?.roles),
  );
  const refreshChart = () => {
    if (props.updatedDttm) {
      props.forceRefresh(props.slice.slice_id, props.reportViewerId);
    }
  };
  ...
  // @z-index-below-reportViewer-header (100) - 1 = 99 for !isFullSize and 101 for isFullSize
  ...
  const menu = (
    ...
    >
      ...
      >
        {t('Force refresh')}
        <RefreshTooltip data-test="reportViewer-slice-refresh-tooltip">
          {refreshTooltip}
        </RefreshTooltip>
      </Menu.Item>
      ...
      {supersetCanShare && (
        ...
        >
          <ShareMenuItems
            ...
            reportViewerComponentId={componentId}
