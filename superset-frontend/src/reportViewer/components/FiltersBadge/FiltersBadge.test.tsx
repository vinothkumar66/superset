...
import { reportViewerWithFilter } from 'spec/fixtures/mockReportViewerLayout';

jest.mock(
  'src/reportViewer/components/FiltersBadge/DetailsPanel',
  () =>
...
buildActiveFilters({
  reportViewerFilters,
  components: reportViewerWithFilter,
});

describe('for reportViewer filters', () => {
  test('does not show number when there are no active filters', () => {
    const store = getMockStoreWithFilters();
    // start with basic reportViewer state, dispatch an event to simulate query completion
    store.dispatch({
