...
import { setDirectPathToChild } from 'src/reportViewer/actions/reportViewerState';
...
import { Chart, ReportViewerLayout, RootState } from '../../types';
...
export const FiltersBadge = ({ chartId }: FiltersBadgeProps) => {
  ...
  const reportViewerFilters = useSelector<RootState, any>(
    state => state.reportViewerFilters,
  );
  ...
  const present = useSelector<RootState, ReportViewerLayout>(
  ...
  const [reportViewerIndicators, setReportViewerIndicators] = useState<Indicator[]>(
    indicatorsInitialState,
  );
  ...
  const prevReportViewerFilters = usePrevious(reportViewerFilters);
  ...
  useEffect(() => {
    if (!showIndicators && reportViewerIndicators.length > 0) {
      setReportViewerIndicators(indicatorsInitialState);
      ...
      if (
        ...
        reportViewerFilters !== prevReportViewerFilters ||
        datasources !== prevDatasources
      ) {
        setReportViewerIndicators(
          selectIndicatorsForChart(
            chartId,
            reportViewerFilters,
            datasources,
    ...
    chartId,
    reportViewerFilters,
    reportViewerIndicators.length,
    datasources,
    prevChart?.queriesResponse,
    prevChartStatus,
    prevReportViewerFilters,
    prevDatasources,
    showIndicators,
  ]);

  const prevNativeFilters = usePrevious(nativeFilters);
  const prevReportViewerLayout = usePrevious(present);
  ...
  useEffect(() => {
    if (!showIndicators && nativeIndicators.length > 0) {
      ...
      if (
        ...
        present !== prevReportViewerLayout ||
  ...
  const indicators = useMemo(
    () =>
      uniqWith(
        sortByStatus([...reportViewerIndicators, ...nativeIndicators]),
        (ind1, ind2) =>
    ...
    [reportViewerIndicators, nativeIndicators],
  );
