...
import { overwriteConfirmMetadata } from 'spec/fixtures/mockReportViewerState';
...
test('requests update reportViewer api when save button is clicked', async () => {
  const updateReportViewerEndpoint = `glob:*/api/v1/reportViewer/${overwriteConfirmMetadata.reportViewerId}`;
  fetchMock.put(updateReportViewerEndpoint, {
    id: overwriteConfirmMetadata.reportViewerId,
  ...
  expect(fetchMock.calls(updateReportViewerEndpoint)).toHaveLength(0);
  fireEvent.click(saveButton);
  expect(fetchMock.calls(updateReportViewerEndpoint)).toHaveLength(0);
  mockAllIsIntersecting(true);
  fireEvent.click(saveButton);
  await waitFor(() =>
    expect(fetchMock.calls(updateReportViewerEndpoint)?.[0]?.[1]?.body).toEqual(
