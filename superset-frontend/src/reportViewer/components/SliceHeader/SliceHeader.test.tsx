...
jest.mock('src/reportViewer/components/SliceHeaderControls', () => ({
  __esModule: true,
  default: (props: any) => (
    <div
      ...
      data-reportViewer-id={props.reportViewerId}
...
jest.mock('src/reportViewer/components/FiltersBadge', () => ({
  __esModule: true,
  default: (props: any) => (
...
test('Should render click to edit prompt and run onExploreChart on click', async () => {
  ...
  const history = createMemoryHistory({
    initialEntries: ['/superset/reportViewer/1/'],
  });
  render(
...
test('Should not render click to edit prompt and run onExploreChart on click if supersetCanExplore=false', () => {
  ...
  userEvent.click(screen.getByText('Vaccine Candidates per Phase'));
  expect(history.location.pathname).toMatch('/superset/reportViewer');
});
...
test('Add extension to SliceHeader', () => {
  ...
  extensionsRegistry.set('reportViewer.slice.header', () => (
