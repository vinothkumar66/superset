...
import { EmbeddedReportViewer } from 'src/reportViewer/types';
...
export const ReportViewerEmbedControls = ({ reportViewerId, onHide }: Props) => {
  ...
  const [embedded, setEmbedded] = useState<EmbeddedReportViewer | null>(null); // the embedded reportViewer config
  const [allowedDomains, setAllowedDomains] = useState<string>('');

  const endpoint = `/api/v1/reportViewer/${reportViewerId}/embedded`;
  ...
  const enableEmbedded = useCallback(() => {
    setLoading(true);
    makeApi<EmbeddedApiPayload, { result: EmbeddedReportViewer }>({
      method: 'POST',
      endpoint,
    })({
  ...
  useEffect(() => {
    setReady(false);
    makeApi<{}, { result: EmbeddedReportViewer }>({
      method: 'GET',
      endpoint,
    })({})
      .catch(err => {
        if ((err as SupersetApiError).status === 404) {
          // 404 just means the reportViewer isn't currently embedded
          return { result: null };
        }
    ...
    <>
      {embedded ? (
        DocsConfigDetails ? (
          ...
          <p>
            {t(
              'This reportViewer is ready to embed. In your application, pass the following id to the SDK:',
            )}
            <br />
        ...
        <p>
          {t(
            'Configure this reportViewer to embed it into an external web application.',
          )}
        </p>
      )}
      <p>
      ...
      <FormItem>
        <label htmlFor="allowed-domains">
          ...
          <InfoTooltipWithTrigger
            tooltip={t(
              'A list of domain names that can embed this reportViewer. Leaving this field empty will allow embedding from any domain.',
            )}
          />
        </label>
        <Input
...
const ReportViewerEmbedModal = (props: Props) => {
  const { show, onHide } = props;
  const ReportViewerEmbedModalExtension = extensionsRegistry.get('embedded.modal');

  return ReportViewerEmbedModalExtension ? (
    <ReportViewerEmbedModalExtension {...props} />
  ) : (
    <Modal show={show} onHide={onHide} title={t('Embed')} hideFooter>
      <ReportViewerEmbedControls {...props} />
    </Modal>
  );
};

export default ReportViewerEmbedModal;
