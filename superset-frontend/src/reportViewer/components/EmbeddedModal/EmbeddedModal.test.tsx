...
import ReportViewerEmbedModal from './index';

const defaultResponse = {
  result: { uuid: 'uuid', reportViewer_id: '1', allowed_domains: ['example.com'] },
};
...
const setup = () => {
  render(<ReportViewerEmbedModal {...defaultProps} />, { useRedux: true });
  resetMockApi();
};
...
test('renders the correct actions when reportViewer is ready to embed', async () => {
  setup();
  expect(
...
test('renders the correct actions when reportViewer is not ready to embed', async () => {
  setMockApiNotFound();
  setup();
  expect(
...
test('adds extension to ReportViewerEmbedModal', async () => {
  const extensionsRegistry = getExtensionsRegistry();

  extensionsRegistry.set('embedded.modal', () => (
    <>reportViewer.embed.modal.extension component</>
  ));
  ...
  expect(
    await screen.findByText('reportViewer.embed.modal.extension component'),
  ).toBeInTheDocument();
});
