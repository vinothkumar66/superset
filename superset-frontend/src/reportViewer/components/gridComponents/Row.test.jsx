...
import Row from 'src/reportViewer/components/gridComponents/Row';
import { REPORTVIEWER_GRID_ID } from 'src/reportViewer/util/constants';

import { getMockStore } from 'spec/fixtures/mockStore';
import { reportViewerLayout as mockLayout } from 'spec/fixtures/mockReportViewerLayout';
...
jest.mock(
  ...
  () =>
    ({ availableColumnCount, depth }) => (
      <div data-test="mock-reportViewer-component" depth={depth}>
...
jest.mock(
  'src/reportViewer/components/menu/WithPopoverMenu',
  () =>
...
const props = {
  id: 'ROW_ID',
  parentId: REPORTVIEWER_GRID_ID,
  component: mockLayout.present.ROW_ID,
  parentComponent: mockLayout.present[REPORTVIEWER_GRID_ID],
  index: 0,
  depth: 2,
  editMode: false,
...
test('should pass appropriate availableColumnCount to children', () => {
  const { getByTestId } = setup();
  expect(getByTestId('mock-reportViewer-component')).toHaveTextContent(
...
test('should increment the depth of its children', () => {
  const { getByTestId } = setup();
  expect(getByTestId('mock-reportViewer-component')).toHaveAttribute(
    'depth',
    `${props.depth + 1}`,
  );
});
