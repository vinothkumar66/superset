...
import { HYDRATE_REPORTVIEWER } from './hydrate';
import { reportViewerInfoChanged } from './reportViewerInfo';
import { ReportViewerInfo } from '../types';
...
export const setInScopeStatusOfFilters =
  ...
  async (dispatch: Dispatch, getState: () => any) => {
    ...
    // need to update native_filter_configuration in the reportViewer metadata
    const metadata = cloneDeep(getState().reportViewerInfo.metadata);
...
export interface SetBootstrapData {
  type: typeof HYDRATE_REPORTVIEWER;
  data: BootstrapData;
}
