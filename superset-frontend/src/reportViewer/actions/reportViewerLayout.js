...
import { TABS_TYPE, ROW_TYPE } from 'src/reportViewer/util/componentTypes';
import {
  REPORTVIEWER_ROOT_ID,
  NEW_COMPONENTS_SOURCE_ID,
  REPORTVIEWER_HEADER_ID,
} from 'src/reportViewer/util/constants';
import dropOverflowsParent from 'src/reportViewer/util/dropOverflowsParent';
import findParentId from 'src/reportViewer/util/findParentId';
import isInDifferentFilterScopes from 'src/reportViewer/util/isInDifferentFilterScopes';
import { updateLayoutComponents } from './reportViewerFilters';
import { setUnsavedChanges } from './reportViewerState';
...
// that reportViewerState.hasUnsavedChanges is false.
function setUnsavedChangesAfterAction(action) {
  return (...args) =>
    (dispatch, getState) => {
      ...
      const { reportViewerLayout, reportViewerState } = getState();

      const isComponentLevelEvent =
        result.type === UPDATE_COMPONENTS &&
        result.payload &&
        result.payload.nextComponents;
      // trigger reportViewerFilters state update if reportViewer layout is changed.
      if (!isComponentLevelEvent) {
        const components = reportViewerLayout.present;
        dispatch(updateLayoutComponents(components));
      }

      if (!reportViewerState.hasUnsavedChanges) {
...
export function updateReportViewerTitle(text) {
  return (dispatch, getState) => {
    const { reportViewerLayout } = getState();
    dispatch(
      updateComponents({
        [REPORTVIEWER_HEADER_ID]: {
          ...reportViewerLayout.present[REPORTVIEWER_HEADER_ID],
          meta: {
            text,
          },
        },
...
export const REPORTVIEWER_TITLE_CHANGED = 'REPORTVIEWER_TITLE_CHANGED';

// call this one when it's not an undo-able action
export function reportViewerTitleChanged(text) {
  return {
    type: REPORTVIEWER_TITLE_CHANGED,
    text,
  };
}
...
export function resizeComponent({ id, width, height }) {
  return (dispatch, getState) => {
    const { reportViewerLayout: undoableLayout } = getState();
    const { present: reportViewer } = undoableLayout;
    const component = reportViewer[id];
...
export function handleComponentDrop(dropResult) {
  return (dispatch, getState) => {
    const overflowsParent = dropOverflowsParent(
      dropResult,
      getState().reportViewerLayout.present,
    );
    ...
    const droppedOnRoot = destination && destination.id === REPORTVIEWER_ROOT_ID;
    const isNewComponent = source.id === NEW_COMPONENTS_SOURCE_ID;
    const reportViewerRoot = getState().reportViewerLayout.present[REPORTVIEWER_ROOT_ID];
    const rootChildId =
      reportViewerRoot && reportViewerRoot.children ? reportViewerRoot.children[0] : '';
    ...
    if (droppedOnRoot) {
      ...
      // ReportViewer does not allow top-level tab become nested tab, to avoid
    ...
    const { reportViewerFilters, reportViewerLayout: undoableLayout } = getState();
...
export function undoLayoutAction() {
  return (dispatch, getState) => {
    ...
    if (
      reportViewerLayout.past.length === 0 &&
      !reportViewerState.maxUndoHistoryExceeded &&
      !reportViewerState.updatedColorScheme
    ) {
