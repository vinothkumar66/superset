...
import { setUnsavedChanges } from 'src/reportViewer/actions/reportViewerState';
import * as reportViewerFilters from 'src/reportViewer/actions/reportViewerFilters';
import { ADD_TOAST } from 'src/components/MessageToasts/actions';

import {
  REPORTVIEWER_GRID_TYPE,
  ROW_TYPE,
  CHART_TYPE,
  TABS_TYPE,
  TAB_TYPE,
...
describe('reportViewerLayout actions', () => {
  const mockState = {
    reportViewerState: {
  ...
  beforeEach(() => {
    sinon.spy(reportViewerFilters, 'updateLayoutComponents');
  });
  afterEach(() => {
    reportViewerFilters.updateLayoutComponents.restore();
  });
  ...
  describe('updateComponents', () => {
    it('should dispatch an updateLayout action', () => {
      ...
      // update component should not trigger action for reportViewerFilters
      expect(reportViewerFilters.updateLayoutComponents.callCount).toEqual(0);
    });
  ...
  describe('deleteComponents', () => {
    it('should dispatch an deleteComponent action', () => {
      ...
      // delete components should trigger action for reportViewerFilters
      expect(reportViewerFilters.updateLayoutComponents.callCount).toEqual(1);
    });
  ...
  describe('updateReportViewerTitle', () => {
    it('should dispatch an updateComponent action for the header component', () => {
      const { getState, dispatch } = setup();
      const thunk1 = updateReportViewerTitle('new text');
      ...
      // update reportViewer title should not trigger action for reportViewerFilters
      expect(reportViewerFilters.updateLayoutComponents.callCount).toEqual(0);
    });
  });
  ...
  describe('createTopLevelTabs', () => {
    it('should dispatch a createTopLevelTabs action', () => {
      ...
      // create top level tabs should trigger action for reportViewerFilters
      expect(reportViewerFilters.updateLayoutComponents.callCount).toEqual(1);
    });
  ...
  describe('deleteTopLevelTabs', () => {
    it('should dispatch a deleteTopLevelTabs action', () => {
      ...
      // delete top level tabs should trigger action for reportViewerFilters
      expect(reportViewerFilters.updateLayoutComponents.callCount).toEqual(1);
    });
  ...
  describe('resizeComponent', () => {
    ...
    it('should dispatch a setUnsavedChanges action if hasUnsavedChanges=false', () => {
      ...
      // resize components should not trigger action for reportViewerFilters
      expect(reportViewerFilters.updateLayoutComponents.callCount).toEqual(0);
    });
  });
  ...
  describe('handleComponentDrop', () => {
    it('should create a component if it is new', () => {
      ...
      const dropResult = {
        ...
        destination: { id: REPORTVIEWER_GRID_ID, type: REPORTVIEWER_GRID_TYPE },
      ...
      // create components should trigger action for reportViewerFilters
      expect(reportViewerFilters.updateLayoutComponents.callCount).toEqual(1);
    });
    ...
    it('should create top-level tabs if dropped on root', () => {
      ...
      const dropResult = {
        source: { id: NEW_COMPONENTS_SOURCE_ID },
        destination: { id: REPORTVIEWER_ROOT_ID },
        dragging: { id: NEW_ROW_ID, type: ROW_TYPE },
      };
  ...
  describe('undoLayoutAction', () => {
    it('should dispatch a redux-undo .undo() action', () => {
      const { getState, dispatch } = setup({
        reportViewerLayout: { past: ['non-empty'] },
      });
  ...
  describe('redoLayoutAction', () => {
    it('should dispatch a redux-undo .redo() action', () => {
      ...
      // redo/undo should trigger action for reportViewerFilters
      expect(reportViewerFilters.updateLayoutComponents.callCount).toEqual(1);
    });
