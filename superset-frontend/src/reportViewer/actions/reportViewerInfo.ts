...
import { onSave } from './reportViewerState';

export const REPORTVIEWER_INFO_UPDATED = 'REPORTVIEWER_INFO_UPDATED';
...
// updates partially changed reportViewer info
export function reportViewerInfoChanged(newInfo: { metadata: any }) {
  ...
  return { type: REPORTVIEWER_INFO_UPDATED, newInfo };
}
...
export const saveChartConfiguration =
  ...
  async (dispatch: Dispatch, getState: () => RootState) => {
    ...
    const { id, metadata } = getState().reportViewerInfo;

    // TODO extract this out when makeApi supports url parameters
    const updateReportViewer = makeApi<
      Partial<ReportViewerInfo>,
      { result: ReportViewerInfo }
    >({
      method: 'PUT',
    ...
    try {
      const response = await updateReportViewer({
...
export function saveFilterBarOrientation(orientation: FilterBarOrientation) {
  return async (dispatch: Dispatch, getState: () => RootState) => {
    ...
    const updateReportViewer = makeApi<
      Partial<ReportViewerInfo>,
      { result: Partial<ReportViewerInfo>; last_modified_time: number }
    >({
      method: 'PUT',
      endpoint: `/api/v1/reportViewer/${id}`,
    });
    try {
      ...
      const updatedReportViewer = response.result;
      ...
      const errorText = await getErrorText(errorObject, 'reportViewer');
