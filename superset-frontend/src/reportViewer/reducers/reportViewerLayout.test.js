...
} from 'src/reportViewer/reducers/reportViewerLayout';
...
describe('reportViewerLayout reducer', () => {
  ...
  it('should wrap a moved component in a row if need be', () => {
    ...
    const dropResult = {
      source: { id: 'source', type: ROW_TYPE, index: 1 },
      destination: { id: 'destination', type: REPORTVIEWER_GRID_TYPE, index: 0 },
      dragging: { id: 'toMove', type: CHART_TYPE },
    };
  ...
  it('should add top-level tabs from a new tabs component, moving grid children to new tab', () => {
    ...
    expect(result[REPORTVIEWER_ROOT_ID].children[0]).toBe(tabsComponent.id);
    expect(result[tabsComponent.id].children[0]).toBe(tabComponent.id);
    expect(result[tabComponent.id].children[0]).toBe('child');
    expect(result[REPORTVIEWER_GRID_ID].children).toHaveLength(0);
  });
  ...
  it('should add top-level tabs from an existing tabs component, moving grid children to new tab', () => {
    ...
    const dropResult = {
      source: { id: REPORTVIEWER_GRID_ID, type: REPORTVIEWER_GRID_TYPE, index: 1 },
      destination: {
        id: REPORTVIEWER_ROOT_ID,
        type: REPORTVIEWER_ROOT_TYPE,
        index: 0,
    ...
    expect(result[REPORTVIEWER_ROOT_ID].children[0]).toBe('tabs');
  ...
  it('should create a component', () => {
    ...
    const newId = result[REPORTVIEWER_GRID_ID].children[1];
    expect(result[REPORTVIEWER_GRID_ID].children).toHaveLength(2);
