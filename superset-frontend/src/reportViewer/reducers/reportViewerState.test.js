...
import reportViewerStateReducer from 'src/reportViewer/reducers/reportViewerState';

describe('reportViewerState reducer', () => {
  it('should return initial state', () => {
    expect(reportViewerStateReducer(undefined, {})).toEqual({});
  });
  ...
  it('should set hasUnsavedChanges', () => {
    expect(reportViewerStateReducer({}, { type: ON_CHANGE })).toEqual({
  ...
  it('should set unsaved changes, max undo history, and editMode to false on save', () => {
    const result = reportViewerStateReducer(
      { hasUnsavedChanges: true },
      { type: ON_SAVE },
    );
  ...
  it('should clear the focused filter field', () => {
    ...
    const cleared = reportViewerStateReducer(initState, {
  ...
  it('should only clear focused filter when the fields match', () => {
    // reportViewer only has 1 focused filter field at a time,
    ...
    const step1 = reportViewerStateReducer(initState, {
      type: SET_FOCUSED_FILTER_FIELD,
      chartId: 2,
      column: 'column_2',
    });
    const step2 = reportViewerStateReducer(step1, {
      type: UNSET_FOCUSED_FILTER_FIELD,
      chartId: 1,
      column: 'column_1',
    });
